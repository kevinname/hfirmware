...
static void init_coef_vlc(VLC *vlc, uint16_t **prun_table,
    uint16_t **plevel_table, const CoefVLCTable *vlc_table, int tab) {
  int n = vlc_table->n;
...
int wma_decode_init(WMADecodeContext* s) {
  ...
  s->sample_rate = s->wfx->rate;
  s->nb_channels = s->wfx->channels;
  s->bit_rate = s->wfx->bitrate;
  s->block_align = s->wfx->blockalign;

  s->coefs = &coefsarray;
  s->frame_out = &frame_out_buf;

  if (s->wfx->codec_id == ASF_CODEC_ID_WMAV1) {
    s->version = 1;
  } else if (s->wfx->codec_id == ASF_CODEC_ID_WMAV2) {
    s->version = 2;
  } else {
  ...
  //flags1 = 0;
  flags2 = 0;
  extradata = s->wfx->data;
  if (s->version == 1 && s->wfx->datalen >= 4) {
    //flags1 = extradata[0] | (extradata[1] << 8);
    flags2 = extradata[2] | (extradata[3] << 8);
  } else if (s->version == 2 && s->wfx->datalen >= 6) {
    //flags1 = extradata[0] | (extradata[1] << 8) |
  ...
  /* compute the scale factor band sizes for each MDCT block size */
  {
    ...
    for (k = 0; k < s->nb_block_sizes; ++k) {
      ...
      s->high_band_start[k] =
          fixtoi32( fixdiv32(tmp1, tmp2) * (block_len>>1) +0x8000);
...
#if 0 //remove by lihongwei, for reduce code size
  ...
  if (s->use_exp_vlc) {
    ...
    s->exp_vlc.table = s->vlcbuf3;
    s->exp_vlc.table_allocated = VLCBUF3SIZE;

    init_vlc(&s->exp_vlc, EXPVLCBITS, sizeof(scale_huffbits), scale_huffbits, 1,
        1, scale_huffcodes, 4, 4, 0);
  ...
  runtabarray[0] = s->runtab0;
  runtabarray[1] = s->runtab1;
  levtabarray[0] = s->levtab0;
  levtabarray[1] = s->levtab1;

  s->coef_vlc[0].table = s->vlcbuf1;
  s->coef_vlc[0].table_allocated = VLCBUF1SIZE;
  s->coef_vlc[1].table = s->vlcbuf2;
...
static void wma_lsp_to_curve(WMADecodeContext *s, fixed32 *out,
    fixed32 *val_max_ptr, int n, fixed32 *lsp) {
  int i, j;
...
static void decode_exp_lsp(WMADecodeContext *s, int ch) {
  ...
  wma_lsp_to_curve(s, s->exponents[ch], &s->max_exponent[ch], s->block_len,
      lsp_coefs);
}
...
static int wma_decode_block(WMADecodeContext *s, int32_t *scratch_buffer) {
        ...
        for (i = 0; i < n; ++i) {
          if (s->high_band_coded[ch][i]) {
            if (val == (int) 0x80000000) {
              ...
              code = get_vlc2(&s->gb, s->hgain_vlc.table, HGAINVLCBITS,
                  HGAINMAX);
              if (code < 0) {
                return -6;
              }
  ...
  for (ch = 0; ch < s->nb_channels; ++ch) {
    if (s->channel_coded[ch]) {
      ...
      if (s->use_noise_coding) {
        ...
        mult = fixdiv64(pow_table[total_gain + 20],
        ...
        for (i = 0; i < s->coefs_start; ++i) {
          *coefs++ = fixmul32(
              (fixmul32(s->noise_table[s->noise_index],
        ...
        for (j = -1; j < n1; ++j) {
          if (j < 0) {
            ...
            n =
                s->exponent_high_bands[s->frame_len_bits - s->block_len_bits][j];
          }
          if (j >= 0 && s->high_band_coded[ch][j]) {
            ...
            mult1 = fixdiv64(mult1,
                fixmul32(s->max_exponent[ch], s->noise_mult));
            mult1 = mult1 * mdct_norm >> PRECISION;
            for (i = 0; i < n; ++i) {
              noise = s->noise_table[s->noise_index];
              s->noise_index = (s->noise_index + 1) & (NOISE_TAB_SIZE - 1);
              *coefs++ = fixmul32(
                  (fixmul32(exponents[i << bsize >> esize], noise) >> 4),
  ...
  for (ch = 0; ch < s->nb_channels; ++ch) {
    if (s->channel_coded[ch]) {
      ...
      mdct_backward(
          (1 << (s->block_len_bits + 1)), (int32_t*)(*(s->coefs))[ch], (int32_t*)scratch_buffer);
...
static int wma_decode_frame(WMADecodeContext *s, int32_t *samples) {
  ...
  for (;;) {
    ...
    if (ret < 0) {
      //debug("wma_decode_block failed with code %d\n", ret);
      return -1;
    }
    if (ret) {
      break;
...
int wma_decode_superframe_init(WMADecodeContext* s, const uint8_t *buf, /*input*/
int buf_size) {
  if (buf_size == 0) {
    s->last_superframe_len = 0;
    return 0;
 ...
 */
int wma_decode_superframe_frame(WMADecodeContext* s, int32_t* samples, /*output*/
...
int buf_size) {
  ...
  if ((s->use_bit_reservoir) && (s->current_frame == 0)) {
    if (s->last_superframe_len > 0) {
      ...
      if ((s->last_superframe_len + ((s->bit_offset + 7) >> 3))
          ...
          {
        //debug("superframe size too large error\n");
        goto fail;
      }
  ...
  if ((s->use_bit_reservoir) && (s->current_frame == s->nb_frames)) {
    ...
    pos = get_bits_count(&s->gb)
        + ((s->bit_offset + 4 + 4 + s->byte_offset_bits + 3) & ~7);
    s->last_bitoffset = pos & 7;
    pos >>= 3;
    len = buf_size - pos;
    if (len > MAX_CODED_SUPERFRAME_SIZE || len < 0) {
      //debug("superframe size too large error after decoding\n");
      goto fail;
    }
