...
typedef struct WMADecodeContext WMADecodeContext;
...
struct id3_tag
{
  ...
  size_t first_frame_offset; /* Byte offset to first real MP3 frame.
  Used for skipping leading garbage to
  avoid gaps between tracks. */

  uint32_t frame_count;   /* number of frames in the file (if VBR) */
  size_t offset;        /* bytes played */
...
{
	uint32_t length;
	uint32_t position; /* current position in the buffer */
  
	uint8_t *buffer;
	size_t buffer_size;
  uint32_t read_buf_ptr;

  WMADecodeContext *dec;
...
struct WMADecodeContext
{
  ...
  //variables that replace malloced stuff
