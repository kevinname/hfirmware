...
static void UnpackSFMPEG1(BitStreamInfo *bsi, SideInfoSub *sis,
    ScaleFactorInfoSub *sfis, int *scfsi, int gr, ScaleFactorInfoSub *sfisGr0) {
  int sfb;
  int slen0, slen1;
...
static void UnpackSFMPEG2(BitStreamInfo *bsi, SideInfoSub *sis,
    ScaleFactorInfoSub *sfis, int ch, int modeExt, ScaleFactorJS *sfjs) {
...
int UnpackScaleFactors(MP3DecInfo *mp3DecInfo, unsigned char *buf,
  ...
  if (fh->ver == MPEG1)
    ...
    UnpackSFMPEG2(bsi, &si->sis[gr][ch], &sfi->sfis[gr][ch], ch, fh->modeExt,
        &sfi->sfjs);
