...
MP3DecInfo *AllocateBuffers(void) {
  ...
  mp3DecInfo = (MP3DecInfo *) hs_malloc(sizeof(MP3DecInfo), __MT_GENERAL);
  if (!mp3DecInfo)
    return 0;
  ClearBuffer(mp3DecInfo, sizeof(MP3DecInfo));

  fh = (FrameHeader *) hs_malloc(sizeof(FrameHeader), __MT_GENERAL);
  si = (SideInfo *) hs_malloc(sizeof(SideInfo), __MT_GENERAL);
  sfi = (ScaleFactorInfo *) hs_malloc(sizeof(ScaleFactorInfo), __MT_GENERAL);
  hi = (HuffmanInfo *) hs_malloc(sizeof(HuffmanInfo), __MT_GENERAL);
  di = (DequantInfo *) hs_malloc(sizeof(DequantInfo), __MT_GENERAL);
  mi = (IMDCTInfo *) hs_malloc(sizeof(IMDCTInfo), __MT_GENERAL);
  sbi = (SubbandInfo *) hs_malloc(sizeof(SubbandInfo), __MT_GENERAL);
...
#define SAFE_FREE(x)	{if (x) hs_free(x);	(x) = 0;}	/* helper macro */
