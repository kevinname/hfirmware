...
/* ULP HCI Event */
#define HCI_BLE_META_EVENT                  0x03E
/* ULP Event sub code */
...
char *acl_cid_name[7] = {"Unknow", "ACL-U", "CLC", "AMP", "ATT", "LE-U", "SMP"};
...
static const char*  dump_hci_cmd_opcode(uint16_t opcode)
{
    switch (opcode)
    {
        /* Following opcode is used only in command complete event for flow control */
        CASE_RETURN_STR(HCI_COMMAND_NONE)

        /* Commands of HCI_GRP_LINK_CONTROL_CMDS group */
        CASE_RETURN_STR(HCI_INQUIRY)
        CASE_RETURN_STR(HCI_INQUIRY_CANCEL)
        CASE_RETURN_STR(HCI_PERIODIC_INQUIRY_MODE)
        CASE_RETURN_STR(HCI_EXIT_PERIODIC_INQUIRY_MODE)
        CASE_RETURN_STR(HCI_CREATE_CONNECTION)
        CASE_RETURN_STR(HCI_DISCONNECT)
        CASE_RETURN_STR(HCI_ADD_SCO_CONNECTION)
        CASE_RETURN_STR(HCI_CREATE_CONNECTION_CANCEL)
        CASE_RETURN_STR(HCI_ACCEPT_CONNECTION_REQUEST)
        CASE_RETURN_STR(HCI_REJECT_CONNECTION_REQUEST)
        CASE_RETURN_STR(HCI_LINK_KEY_REQUEST_REPLY)
        CASE_RETURN_STR(HCI_LINK_KEY_REQUEST_NEG_REPLY)
        CASE_RETURN_STR(HCI_PIN_CODE_REQUEST_REPLY)
        CASE_RETURN_STR(HCI_PIN_CODE_REQUEST_NEG_REPLY)
        CASE_RETURN_STR(HCI_CHANGE_CONN_PACKET_TYPE)
        CASE_RETURN_STR(HCI_AUTHENTICATION_REQUESTED)
        CASE_RETURN_STR(HCI_SET_CONN_ENCRYPTION)
        CASE_RETURN_STR(HCI_CHANGE_CONN_LINK_KEY)
        CASE_RETURN_STR(HCI_MASTER_LINK_KEY)
        CASE_RETURN_STR(HCI_RMT_NAME_REQUEST)
        CASE_RETURN_STR(HCI_RMT_NAME_REQUEST_CANCEL)
        CASE_RETURN_STR(HCI_READ_RMT_FEATURES)
        CASE_RETURN_STR(HCI_READ_RMT_EXT_FEATURES)
        CASE_RETURN_STR(HCI_READ_RMT_VERSION_INFO)
        CASE_RETURN_STR(HCI_READ_RMT_CLOCK_OFFSET)
        CASE_RETURN_STR(HCI_READ_LMP_HANDLE)
        CASE_RETURN_STR(HCI_SETUP_ESCO_CONNECTION)
        CASE_RETURN_STR(HCI_ACCEPT_ESCO_CONNECTION)
        CASE_RETURN_STR(HCI_REJECT_ESCO_CONNECTION)
        CASE_RETURN_STR(HCI_IO_CAPABILITY_RESPONSE)
        CASE_RETURN_STR(HCI_USER_CONF_REQUEST_REPLY)
        CASE_RETURN_STR(HCI_USER_CONF_VALUE_NEG_REPLY)
        CASE_RETURN_STR(HCI_USER_PASSKEY_REQ_REPLY)
        CASE_RETURN_STR(HCI_USER_PASSKEY_REQ_NEG_REPLY)
        CASE_RETURN_STR(HCI_REM_OOB_DATA_REQ_REPLY)
        CASE_RETURN_STR(HCI_REM_OOB_DATA_REQ_NEG_REPLY)
        CASE_RETURN_STR(HCI_IO_CAP_REQ_NEG_REPLY)

        /* AMP HCI */
        CASE_RETURN_STR(HCI_CREATE_PHYSICAL_LINK)
        CASE_RETURN_STR(HCI_ACCEPT_PHYSICAL_LINK)
        CASE_RETURN_STR(HCI_DISCONNECT_PHYSICAL_LINK)
        CASE_RETURN_STR(HCI_CREATE_LOGICAL_LINK)
        CASE_RETURN_STR(HCI_ACCEPT_LOGICAL_LINK)
        CASE_RETURN_STR(HCI_DISCONNECT_LOGICAL_LINK)
        CASE_RETURN_STR(HCI_LOGICAL_LINK_CANCEL)
        CASE_RETURN_STR(HCI_FLOW_SPEC_MODIFY)

        CASE_RETURN_STR(HCI_ENH_SETUP_ESCO_CONNECTION)
        CASE_RETURN_STR(HCI_ENH_ACCEPT_ESCO_CONNECTION)

        /* ConnectionLess Broadcast */
        CASE_RETURN_STR(HCI_TRUNCATED_PAGE)
        CASE_RETURN_STR(HCI_TRUNCATED_PAGE_CANCEL)
        CASE_RETURN_STR(HCI_SET_CLB)
        CASE_RETURN_STR(HCI_RECEIVE_CLB)
        CASE_RETURN_STR(HCI_START_SYNC_TRAIN)
        CASE_RETURN_STR(HCI_RECEIVE_SYNC_TRAIN)

        /* Commands of HCI_GRP_LINK_POLICY_CMDS */
        CASE_RETURN_STR(HCI_HOLD_MODE)
        CASE_RETURN_STR(HCI_SNIFF_MODE)
        CASE_RETURN_STR(HCI_EXIT_SNIFF_MODE)
        CASE_RETURN_STR(HCI_PARK_MODE)
        CASE_RETURN_STR(HCI_EXIT_PARK_MODE)
        CASE_RETURN_STR(HCI_QOS_SETUP)
        CASE_RETURN_STR(HCI_ROLE_DISCOVERY)
        CASE_RETURN_STR(HCI_SWITCH_ROLE)
        CASE_RETURN_STR(HCI_READ_POLICY_SETTINGS)
        CASE_RETURN_STR(HCI_WRITE_POLICY_SETTINGS)
        CASE_RETURN_STR(HCI_READ_DEF_POLICY_SETTINGS)
        CASE_RETURN_STR(HCI_WRITE_DEF_POLICY_SETTINGS)
        CASE_RETURN_STR(HCI_FLOW_SPECIFICATION)
        CASE_RETURN_STR(HCI_SNIFF_SUB_RATE)

        /* Commands of HCI_GRP_HOST_CONT_BASEBAND_CMDS */
        CASE_RETURN_STR(HCI_SET_EVENT_MASK)
        CASE_RETURN_STR(HCI_RESET)
        CASE_RETURN_STR(HCI_SET_EVENT_FILTER)
        CASE_RETURN_STR(HCI_FLUSH)
        CASE_RETURN_STR(HCI_READ_PIN_TYPE)
        CASE_RETURN_STR(HCI_WRITE_PIN_TYPE)
        CASE_RETURN_STR(HCI_CREATE_NEW_UNIT_KEY)
        CASE_RETURN_STR(HCI_GET_MWS_TRANS_LAYER_CFG)
        CASE_RETURN_STR(HCI_READ_STORED_LINK_KEY)
        CASE_RETURN_STR(HCI_WRITE_STORED_LINK_KEY)
        CASE_RETURN_STR(HCI_DELETE_STORED_LINK_KEY)
        CASE_RETURN_STR(HCI_CHANGE_LOCAL_NAME)
        CASE_RETURN_STR(HCI_READ_LOCAL_NAME)
        CASE_RETURN_STR(HCI_READ_CONN_ACCEPT_TOUT)
        CASE_RETURN_STR(HCI_WRITE_CONN_ACCEPT_TOUT)
        CASE_RETURN_STR(HCI_READ_PAGE_TOUT)
        CASE_RETURN_STR(HCI_WRITE_PAGE_TOUT)
        CASE_RETURN_STR(HCI_READ_SCAN_ENABLE)
        CASE_RETURN_STR(HCI_WRITE_SCAN_ENABLE)
        CASE_RETURN_STR(HCI_READ_PAGESCAN_CFG)
        CASE_RETURN_STR(HCI_WRITE_PAGESCAN_CFG)
        CASE_RETURN_STR(HCI_READ_INQUIRYSCAN_CFG)
        CASE_RETURN_STR(HCI_WRITE_INQUIRYSCAN_CFG)
        CASE_RETURN_STR(HCI_READ_AUTHENTICATION_ENABLE)
        CASE_RETURN_STR(HCI_WRITE_AUTHENTICATION_ENABLE)
        CASE_RETURN_STR(HCI_READ_ENCRYPTION_MODE)
        CASE_RETURN_STR(HCI_WRITE_ENCRYPTION_MODE)
        CASE_RETURN_STR(HCI_READ_CLASS_OF_DEVICE)
        CASE_RETURN_STR(HCI_WRITE_CLASS_OF_DEVICE)
        CASE_RETURN_STR(HCI_READ_VOICE_SETTINGS)
        CASE_RETURN_STR(HCI_WRITE_VOICE_SETTINGS)
        CASE_RETURN_STR(HCI_READ_AUTO_FLUSH_TOUT)
        CASE_RETURN_STR(HCI_WRITE_AUTO_FLUSH_TOUT)
        CASE_RETURN_STR(HCI_READ_NUM_BCAST_REXMITS)
        CASE_RETURN_STR(HCI_WRITE_NUM_BCAST_REXMITS)
        CASE_RETURN_STR(HCI_READ_HOLD_MODE_ACTIVITY)
        CASE_RETURN_STR(HCI_WRITE_HOLD_MODE_ACTIVITY)
        CASE_RETURN_STR(HCI_READ_TRANSMIT_POWER_LEVEL)
        CASE_RETURN_STR(HCI_READ_SCO_FLOW_CTRL_ENABLE)
        CASE_RETURN_STR(HCI_WRITE_SCO_FLOW_CTRL_ENABLE)
        CASE_RETURN_STR(HCI_SET_HC_TO_HOST_FLOW_CTRL)
        CASE_RETURN_STR(HCI_HOST_BUFFER_SIZE)
        CASE_RETURN_STR(HCI_HOST_NUM_PACKETS_DONE)
        CASE_RETURN_STR(HCI_READ_LINK_SUPER_TOUT)
        CASE_RETURN_STR(HCI_WRITE_LINK_SUPER_TOUT)
        CASE_RETURN_STR(HCI_READ_NUM_SUPPORTED_IAC)
        CASE_RETURN_STR(HCI_READ_CURRENT_IAC_LAP)
        CASE_RETURN_STR(HCI_WRITE_CURRENT_IAC_LAP)
        CASE_RETURN_STR(HCI_READ_PAGESCAN_PERIOD_MODE)
        CASE_RETURN_STR(HCI_WRITE_PAGESCAN_PERIOD_MODE)
        CASE_RETURN_STR(HCI_READ_PAGESCAN_MODE)
        CASE_RETURN_STR(HCI_WRITE_PAGESCAN_MODE)
        CASE_RETURN_STR(HCI_SET_AFH_CHANNELS)

        CASE_RETURN_STR(HCI_READ_INQSCAN_TYPE)
        CASE_RETURN_STR(HCI_WRITE_INQSCAN_TYPE)
        CASE_RETURN_STR(HCI_READ_INQUIRY_MODE)
        CASE_RETURN_STR(HCI_WRITE_INQUIRY_MODE)
        CASE_RETURN_STR(HCI_READ_PAGESCAN_TYPE)
        CASE_RETURN_STR(HCI_WRITE_PAGESCAN_TYPE)
        CASE_RETURN_STR(HCI_READ_AFH_ASSESSMENT_MODE)
        CASE_RETURN_STR(HCI_WRITE_AFH_ASSESSMENT_MODE)
        CASE_RETURN_STR(HCI_READ_EXT_INQ_RESPONSE)
        CASE_RETURN_STR(HCI_WRITE_EXT_INQ_RESPONSE)
        CASE_RETURN_STR(HCI_REFRESH_ENCRYPTION_KEY)
        CASE_RETURN_STR(HCI_READ_SIMPLE_PAIRING_MODE)
        CASE_RETURN_STR(HCI_WRITE_SIMPLE_PAIRING_MODE)
        CASE_RETURN_STR(HCI_READ_LOCAL_OOB_DATA)
        CASE_RETURN_STR(HCI_READ_INQ_TX_POWER_LEVEL)
        CASE_RETURN_STR(HCI_WRITE_INQ_TX_POWER_LEVEL)
        CASE_RETURN_STR(HCI_READ_ERRONEOUS_DATA_RPT)
        CASE_RETURN_STR(HCI_WRITE_ERRONEOUS_DATA_RPT)
        CASE_RETURN_STR(HCI_ENHANCED_FLUSH)
        CASE_RETURN_STR(HCI_SEND_KEYPRESS_NOTIF)

        /* AMP HCI */
        CASE_RETURN_STR(HCI_READ_LOGICAL_LINK_ACCEPT_TIMEOUT)
        CASE_RETURN_STR(HCI_WRITE_LOGICAL_LINK_ACCEPT_TIMEOUT)
        CASE_RETURN_STR(HCI_SET_EVENT_MASK_PAGE_2)
        CASE_RETURN_STR(HCI_READ_LOCATION_DATA)
        CASE_RETURN_STR(HCI_WRITE_LOCATION_DATA)
        CASE_RETURN_STR(HCI_READ_FLOW_CONTROL_MODE)
        CASE_RETURN_STR(HCI_WRITE_FLOW_CONTROL_MODE)
        CASE_RETURN_STR(HCI_READ_BE_FLUSH_TOUT)
        CASE_RETURN_STR(HCI_WRITE_BE_FLUSH_TOUT)
        CASE_RETURN_STR(HCI_SHORT_RANGE_MODE)
        CASE_RETURN_STR(HCI_READ_LE_HOST_SUPPORTED)
        CASE_RETURN_STR(HCI_WRITE_LE_HOST_SUPPORTED)

        /* MWS coexistence */
        CASE_RETURN_STR(HCI_SET_MWS_CHANNEL_PARAMETERS)
        CASE_RETURN_STR(HCI_SET_EXTERNAL_FRAME_CONFIGURATION)
        CASE_RETURN_STR(HCI_SET_MWS_SIGNALING)
        CASE_RETURN_STR(HCI_SET_MWS_TRANSPORT_LAYER)
        CASE_RETURN_STR(HCI_SET_MWS_SCAN_FREQUENCY_TABLE)
        CASE_RETURN_STR(HCI_SET_MWS_PATTERN_CONFIGURATION)

        /* ConnectionLess Broadcast */
        CASE_RETURN_STR(HCI_SET_RESERVED_LT_ADDR)
        CASE_RETURN_STR(HCI_DELETE_RESERVED_LT_ADDR)
        CASE_RETURN_STR(HCI_WRITE_CLB_DATA)
        CASE_RETURN_STR(HCI_WRITE_SYNC_TRAIN_PARAM)
        CASE_RETURN_STR(HCI_READ_SYNC_TRAIN_PARAM)

        /* Commands of HCI_GRP_INFORMATIONAL_PARAMS group */
        CASE_RETURN_STR(HCI_READ_LOCAL_VERSION_INFO)
        CASE_RETURN_STR(HCI_READ_LOCAL_SUPPORTED_CMDS)
        CASE_RETURN_STR(HCI_READ_LOCAL_FEATURES)
        CASE_RETURN_STR(HCI_READ_LOCAL_EXT_FEATURES)
        CASE_RETURN_STR(HCI_READ_BUFFER_SIZE)
        CASE_RETURN_STR(HCI_READ_COUNTRY_CODE)
        CASE_RETURN_STR(HCI_READ_BD_ADDR)
        CASE_RETURN_STR(HCI_READ_DATA_BLOCK_SIZE)
        CASE_RETURN_STR(HCI_READ_LOCAL_SUPPORTED_CODECS)

        /* Commands of HCI_GRP_STATUS_PARAMS group */
        CASE_RETURN_STR(HCI_READ_FAILED_CONTACT_COUNT)
        CASE_RETURN_STR(HCI_RESET_FAILED_CONTACT_COUNT)
        CASE_RETURN_STR(HCI_GET_LINK_QUALITY)
        CASE_RETURN_STR(HCI_READ_RSSI)
        CASE_RETURN_STR(HCI_READ_AFH_CH_MAP)
        CASE_RETURN_STR(HCI_READ_CLOCK)
        CASE_RETURN_STR(HCI_READ_ENCR_KEY_SIZE)

        /* AMP HCI */
        CASE_RETURN_STR(HCI_READ_LOCAL_AMP_INFO)
        CASE_RETURN_STR(HCI_READ_LOCAL_AMP_ASSOC)
        CASE_RETURN_STR(HCI_WRITE_REMOTE_AMP_ASSOC)

        /* Commands of HCI_GRP_TESTING_CMDS group */
        CASE_RETURN_STR(HCI_READ_LOOPBACK_MODE)
        CASE_RETURN_STR(HCI_WRITE_LOOPBACK_MODE)
        CASE_RETURN_STR(HCI_ENABLE_DEV_UNDER_TEST_MODE)
        CASE_RETURN_STR(HCI_WRITE_SIMP_PAIR_DEBUG_MODE)

        /* AMP HCI */
        CASE_RETURN_STR(HCI_ENABLE_AMP_RCVR_REPORTS)
        CASE_RETURN_STR(HCI_AMP_TEST_END)
        CASE_RETURN_STR(HCI_AMP_TEST)

        /* BLE HCI */
        /* Commands of BLE Controller setup and configuration */
        CASE_RETURN_STR(HCI_BLE_SET_EVENT_MASK)
        CASE_RETURN_STR(HCI_BLE_READ_BUFFER_SIZE)
        CASE_RETURN_STR(HCI_BLE_READ_LOCAL_SPT_FEAT)
        CASE_RETURN_STR(HCI_BLE_WRITE_LOCAL_SPT_FEAT)
        CASE_RETURN_STR(HCI_BLE_WRITE_RANDOM_ADDR)
        CASE_RETURN_STR(HCI_BLE_WRITE_ADV_PARAMS)
        CASE_RETURN_STR(HCI_BLE_READ_ADV_CHNL_TX_POWER)
        CASE_RETURN_STR(HCI_BLE_WRITE_ADV_DATA)
        CASE_RETURN_STR(HCI_BLE_WRITE_SCAN_RSP_DATA)
        CASE_RETURN_STR(HCI_BLE_WRITE_ADV_ENABLE)
        CASE_RETURN_STR(HCI_BLE_WRITE_SCAN_PARAMS)
        CASE_RETURN_STR(HCI_BLE_WRITE_SCAN_ENABLE)
        CASE_RETURN_STR(HCI_BLE_CREATE_LL_CONN)
        CASE_RETURN_STR(HCI_BLE_CREATE_CONN_CANCEL)
        CASE_RETURN_STR(HCI_BLE_READ_WHITE_LIST_SIZE)
        CASE_RETURN_STR(HCI_BLE_CLEAR_WHITE_LIST)
        CASE_RETURN_STR(HCI_BLE_ADD_WHITE_LIST)
        CASE_RETURN_STR(HCI_BLE_REMOVE_WHITE_LIST)
        CASE_RETURN_STR(HCI_BLE_UPD_LL_CONN_PARAMS)
        CASE_RETURN_STR(HCI_BLE_SET_HOST_CHNL_CLASS)
        CASE_RETURN_STR(HCI_BLE_READ_CHNL_MAP)
        CASE_RETURN_STR(HCI_BLE_READ_REMOTE_FEAT)
        CASE_RETURN_STR(HCI_BLE_ENCRYPT)
        CASE_RETURN_STR(HCI_BLE_RAND)
        CASE_RETURN_STR(HCI_BLE_START_ENC)
        CASE_RETURN_STR(HCI_BLE_LTK_REQ_REPLY)
        CASE_RETURN_STR(HCI_BLE_LTK_REQ_NEG_REPLY)
        CASE_RETURN_STR(HCI_BLE_READ_SUPPORTED_STATES)

        /* BLE TEST COMMANDS */
        CASE_RETURN_STR(HCI_BLE_RECEIVER_TEST)
        CASE_RETURN_STR(HCI_BLE_TRANSMITTER_TEST)
        CASE_RETURN_STR(HCI_BLE_TEST_END)

        CASE_RETURN_STR(HCI_BLE_RESET)

        default:
            return "Unknown HCI CMD OPCODE";

    }
}

static const char*  dump_hci_event_opcode(uint8_t opcode)
{
    switch (opcode)
    {
        /*  Definitions for HCI Events */
        CASE_RETURN_STR(HCI_INQUIRY_COMP_EVT)
        CASE_RETURN_STR(HCI_INQUIRY_RESULT_EVT)
        CASE_RETURN_STR(HCI_CONNECTION_COMP_EVT)
        CASE_RETURN_STR(HCI_CONNECTION_REQUEST_EVT)
        CASE_RETURN_STR(HCI_DISCONNECTION_COMP_EVT)
        CASE_RETURN_STR(HCI_AUTHENTICATION_COMP_EVT)
        CASE_RETURN_STR(HCI_RMT_NAME_REQUEST_COMP_EVT)
        CASE_RETURN_STR(HCI_ENCRYPTION_CHANGE_EVT)
        CASE_RETURN_STR(HCI_CHANGE_CONN_LINK_KEY_EVT)
        CASE_RETURN_STR(HCI_MASTER_LINK_KEY_COMP_EVT)
        CASE_RETURN_STR(HCI_READ_RMT_FEATURES_COMP_EVT)
        CASE_RETURN_STR(HCI_READ_RMT_VERSION_COMP_EVT)
        CASE_RETURN_STR(HCI_QOS_SETUP_COMP_EVT)
        CASE_RETURN_STR(HCI_COMMAND_COMPLETE_EVT)
        CASE_RETURN_STR(HCI_COMMAND_STATUS_EVT)
        CASE_RETURN_STR(HCI_HARDWARE_ERROR_EVT)
        CASE_RETURN_STR(HCI_FLUSH_OCCURED_EVT)
        CASE_RETURN_STR(HCI_ROLE_CHANGE_EVT)
        CASE_RETURN_STR(HCI_NUM_COMPL_DATA_PKTS_EVT)
        CASE_RETURN_STR(HCI_MODE_CHANGE_EVT)
        CASE_RETURN_STR(HCI_RETURN_LINK_KEYS_EVT)
        CASE_RETURN_STR(HCI_PIN_CODE_REQUEST_EVT)
        CASE_RETURN_STR(HCI_LINK_KEY_REQUEST_EVT)
        CASE_RETURN_STR(HCI_LINK_KEY_NOTIFICATION_EVT)
        CASE_RETURN_STR(HCI_LOOPBACK_COMMAND_EVT)
        CASE_RETURN_STR(HCI_DATA_BUF_OVERFLOW_EVT)
        CASE_RETURN_STR(HCI_MAX_SLOTS_CHANGED_EVT)
        CASE_RETURN_STR(HCI_READ_CLOCK_OFF_COMP_EVT)
        CASE_RETURN_STR(HCI_CONN_PKT_TYPE_CHANGE_EVT)
        CASE_RETURN_STR(HCI_QOS_VIOLATION_EVT)
        CASE_RETURN_STR(HCI_PAGE_SCAN_MODE_CHANGE_EVT)
        CASE_RETURN_STR(HCI_PAGE_SCAN_REP_MODE_CHNG_EVT)
        CASE_RETURN_STR(HCI_FLOW_SPECIFICATION_COMP_EVT)
        CASE_RETURN_STR(HCI_INQUIRY_RSSI_RESULT_EVT)
        CASE_RETURN_STR(HCI_READ_RMT_EXT_FEATURES_COMP_EVT)
        CASE_RETURN_STR(HCI_ESCO_CONNECTION_COMP_EVT)
        CASE_RETURN_STR(HCI_ESCO_CONNECTION_CHANGED_EVT)
        CASE_RETURN_STR(HCI_SNIFF_SUB_RATE_EVT)
        CASE_RETURN_STR(HCI_EXTENDED_INQUIRY_RESULT_EVT)
        CASE_RETURN_STR(HCI_ENCRYPTION_KEY_REFRESH_COMP_EVT)
        CASE_RETURN_STR(HCI_IO_CAPABILITY_REQUEST_EVT)
        CASE_RETURN_STR(HCI_IO_CAPABILITY_RESPONSE_EVT)
        CASE_RETURN_STR(HCI_USER_CONFIRMATION_REQUEST_EVT)
        CASE_RETURN_STR(HCI_USER_PASSKEY_REQUEST_EVT)
        CASE_RETURN_STR(HCI_REMOTE_OOB_DATA_REQUEST_EVT)
        CASE_RETURN_STR(HCI_SIMPLE_PAIRING_COMPLETE_EVT)
        CASE_RETURN_STR(HCI_LINK_SUPER_TOUT_CHANGED_EVT)
        CASE_RETURN_STR(HCI_ENHANCED_FLUSH_COMPLETE_EVT)
        CASE_RETURN_STR(HCI_USER_PASSKEY_NOTIFY_EVT)
        CASE_RETURN_STR(HCI_KEYPRESS_NOTIFY_EVT)
        CASE_RETURN_STR(HCI_RMT_HOST_SUP_FEAT_NOTIFY_EVT)

        /*CASE_RETURN_STR(HCI_GENERIC_AMP_LINK_KEY_NOTIF_EVT  0x3E Removed from spec */
        CASE_RETURN_STR(HCI_PHYSICAL_LINK_COMP_EVT)
        CASE_RETURN_STR(HCI_CHANNEL_SELECTED_EVT)
        CASE_RETURN_STR(HCI_DISC_PHYSICAL_LINK_COMP_EVT)
        CASE_RETURN_STR(HCI_PHY_LINK_LOSS_EARLY_WARNING_EVT)
        CASE_RETURN_STR(HCI_PHY_LINK_RECOVERY_EVT)
        CASE_RETURN_STR(HCI_LOGICAL_LINK_COMP_EVT)
        CASE_RETURN_STR(HCI_DISC_LOGICAL_LINK_COMP_EVT)
        CASE_RETURN_STR(HCI_FLOW_SPEC_MODIFY_COMP_EVT)
        CASE_RETURN_STR(HCI_NUM_COMPL_DATA_BLOCKS_EVT)
        CASE_RETURN_STR(HCI_SHORT_RANGE_MODE_COMPLETE_EVT)
        CASE_RETURN_STR(HCI_AMP_STATUS_CHANGE_EVT)

        /* ULP HCI Event */
        CASE_RETURN_STR(HCI_BLE_META_EVENT)

        /* ConnectionLess Broadcast events */
        CASE_RETURN_STR(HCI_SYNC_TRAIN_COMP_EVT)
        CASE_RETURN_STR(HCI_SYNC_TRAIN_RECEIVED_EVT)
        CASE_RETURN_STR(HCI_CLB_RX_DATA_EVT)
        CASE_RETURN_STR(HCI_CLB_RX_TIMEOUT_EVT)
        CASE_RETURN_STR(HCI_TRUNCATED_PAGE_COMP_EVT)
        CASE_RETURN_STR(HCI_SLAVE_PAGE_RESP_TIMEOUT_EVT)
        CASE_RETURN_STR(HCI_CLB_CHANNEL_CHANGE_EVT)
        CASE_RETURN_STR(HCI_INQUIRY_RESPONSE_NOTIF)

        default:
            return "Unknown HCI EVENT OPCODE";
    }
}

static const char* dump_hci_err_code(uint8_t err_code)
{
    switch (err_code)
    {
        CASE_RETURN_STR(HCI_SUCCESS)
        CASE_RETURN_STR(HCI_ERR_ILLEGAL_COMMAND)
        CASE_RETURN_STR(HCI_ERR_NO_CONNECTION)
        CASE_RETURN_STR(HCI_ERR_HW_FAILURE)
        CASE_RETURN_STR(HCI_ERR_PAGE_TIMEOUT)
        CASE_RETURN_STR(HCI_ERR_AUTH_FAILURE)
        CASE_RETURN_STR(HCI_ERR_KEY_MISSING)
        CASE_RETURN_STR(HCI_ERR_MEMORY_FULL)
        CASE_RETURN_STR(HCI_ERR_CONNECTION_TOUT)
        CASE_RETURN_STR(HCI_ERR_MAX_NUM_OF_CONNECTIONS)
        CASE_RETURN_STR(HCI_ERR_MAX_NUM_OF_SCOS)
        CASE_RETURN_STR(HCI_ERR_CONNECTION_EXISTS)
        CASE_RETURN_STR(HCI_ERR_COMMAND_DISALLOWED)
        CASE_RETURN_STR(HCI_ERR_HOST_REJECT_RESOURCES)
        CASE_RETURN_STR(HCI_ERR_HOST_REJECT_SECURITY)
        CASE_RETURN_STR(HCI_ERR_HOST_REJECT_DEVICE)
        CASE_RETURN_STR(HCI_ERR_HOST_TIMEOUT)
        CASE_RETURN_STR(HCI_ERR_UNSUPPORTED_VALUE)
        CASE_RETURN_STR(HCI_ERR_ILLEGAL_PARAMETER_FMT)
        CASE_RETURN_STR(HCI_ERR_PEER_USER)
        CASE_RETURN_STR(HCI_ERR_PEER_LOW_RESOURCES)
        CASE_RETURN_STR(HCI_ERR_PEER_POWER_OFF)
        CASE_RETURN_STR(HCI_ERR_CONN_CAUSE_LOCAL_HOST)
        CASE_RETURN_STR(HCI_ERR_REPEATED_ATTEMPTS)
        CASE_RETURN_STR(HCI_ERR_PAIRING_NOT_ALLOWED)
        CASE_RETURN_STR(HCI_ERR_UNKNOWN_LMP_PDU)
        CASE_RETURN_STR(HCI_ERR_UNSUPPORTED_REM_FEATURE)
        CASE_RETURN_STR(HCI_ERR_SCO_OFFSET_REJECTED)
        CASE_RETURN_STR(HCI_ERR_SCO_INTERVAL_REJECTED)
        CASE_RETURN_STR(HCI_ERR_SCO_AIR_MODE)
        CASE_RETURN_STR(HCI_ERR_INVALID_LMP_PARAM)
        CASE_RETURN_STR(HCI_ERR_UNSPECIFIED)
        CASE_RETURN_STR(HCI_ERR_UNSUPPORTED_LMP_FEATURE)
        CASE_RETURN_STR(HCI_ERR_ROLE_CHANGE_NOT_ALLOWED)
        CASE_RETURN_STR(HCI_ERR_LMP_RESPONSE_TIMEOUT)
        CASE_RETURN_STR(HCI_ERR_LMP_ERR_TRANS_COLLISION)
        CASE_RETURN_STR(HCI_ERR_LMP_PDU_NOT_ALLOWED)
        CASE_RETURN_STR(HCI_ERR_ENCRY_MODE_NOT_ACCEPTABLE)
        CASE_RETURN_STR(HCI_ERR_UNIT_KEY_USED)
        CASE_RETURN_STR(HCI_ERR_QOS_NOT_SUPPORTED)
        CASE_RETURN_STR(HCI_ERR_INSTANT_PASSED)
        CASE_RETURN_STR(HCI_ERR_PAIRING_WITH_UNIT_KEY_NOT_SUPPORTED)
        CASE_RETURN_STR(HCI_ERR_DIFF_TRANSACTION_COLLISION)
        CASE_RETURN_STR(HCI_ERR_UNDEFINED_0x2B)
        CASE_RETURN_STR(HCI_ERR_QOS_UNACCEPTABLE_PARAM)
        CASE_RETURN_STR(HCI_ERR_QOS_REJECTED)
        CASE_RETURN_STR(HCI_ERR_CHAN_CLASSIF_NOT_SUPPORTED)
        CASE_RETURN_STR(HCI_ERR_INSUFFCIENT_SECURITY)
        CASE_RETURN_STR(HCI_ERR_PARAM_OUT_OF_RANGE)
        CASE_RETURN_STR(HCI_ERR_UNDEFINED_0x31)
        CASE_RETURN_STR(HCI_ERR_ROLE_SWITCH_PENDING)
        CASE_RETURN_STR(HCI_ERR_UNDEFINED_0x33)
        CASE_RETURN_STR(HCI_ERR_RESERVED_SLOT_VIOLATION)
        CASE_RETURN_STR(HCI_ERR_ROLE_SWITCH_FAILED)
        CASE_RETURN_STR(HCI_ERR_INQ_RSP_DATA_TOO_LARGE)
        CASE_RETURN_STR(HCI_ERR_SIMPLE_PAIRING_NOT_SUPPORTED)
        CASE_RETURN_STR(HCI_ERR_HOST_BUSY_PAIRING)
        CASE_RETURN_STR(HCI_ERR_REJ_NO_SUITABLE_CHANNEL)
        CASE_RETURN_STR(HCI_ERR_CONTROLLER_BUSY)
        CASE_RETURN_STR(HCI_ERR_UNACCEPT_CONN_INTERVAL)
        CASE_RETURN_STR(HCI_ERR_DIRECTED_ADVERTISING_TIMEOUT)
        CASE_RETURN_STR(HCI_ERR_CONN_TOUT_DUE_TO_MIC_FAILURE)
        CASE_RETURN_STR(HCI_ERR_CONN_FAILED_ESTABLISHMENT)
        CASE_RETURN_STR(HCI_ERR_MAC_CONNECTION_FAILED)
        CASE_RETURN_STR(HCI_ERR_LT_ADDR_ALREADY_IN_USE)
        CASE_RETURN_STR(HCI_ERR_LT_ADDR_NOT_ALLOCATED)
        CASE_RETURN_STR(HCI_ERR_CLB_NOT_ENABLED)
        CASE_RETURN_STR(HCI_ERR_CLB_DATA_TOO_BIG)
...
static const char* dump_hci_le_meta_evt(uint8_t evt)
{
    switch (evt)
    {
        CASE_RETURN_STR(HCI_BLE_CONN_COMPLETE_EVT)
        CASE_RETURN_STR(HCI_BLE_ADV_PKT_RPT_EVT)
        CASE_RETURN_STR(HCI_BLE_LL_CONN_PARAM_UPD_EVT)
        CASE_RETURN_STR(HCI_BLE_READ_REMOTE_FEAT_CMPL_EVT)
        CASE_RETURN_STR(HCI_BLE_LTK_REQ_EVT)

        default:
            return "Unknown HCI LE META EVENT";
    }
}

#define READ_BT_16( buffer, pos) ( ((uint16_t) buffer[pos]) | (((uint16_t)buffer[(pos)+1]) << 8))
#define READ_L2CAP_CHANNEL_ID(buffer) ( READ_BT_16(buffer, 2/*6*/))

static void printf_hexdump(const void *data, int size)
{
  int i;
  if (size > 0) {
    for (i=0; i<size;i++){
      hs_printf("%02X ", ((uint8_t *)data)[i]);
    }
  }
  hs_printf("\r\n");
}
...
void printf_packet(uint8_t packet_type, uint8_t in, uint8_t *header, uint8_t *packet, uint16_t len)
{
  ...
  case HCI_TLPKT_COMMAND:
    hs_printf("CMD-%s\t => ", dump_hci_cmd_opcode(READ_BT_16(header, 0)));
    break;
  case HCI_TLPKT_EVENT:
    hs_printf("EVT-%s\t <= ", dump_hci_event_opcode(*packet));
    break;
  case HCI_TLPKT_ACLDATA:
    hs_printf("ACL-%s\t ", READ_L2CAP_CHANNEL_ID(packet) >= 0x40 ? "Dynamic" : acl_cid_name[READ_L2CAP_CHANNEL_ID(packet)]);
    if (in) {
      hs_printf("<= ");
    } else {
      hs_printf("=> ");
