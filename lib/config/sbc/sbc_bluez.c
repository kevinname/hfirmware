...
__STATIC_INLINE void sbc_init(struct sbc_decoder_state *state,
...
__STATIC_INLINE  void sbc_synthesize_four(struct sbc_decoder_state *state,
...
__STATIC_INLINE void sbc_synthesize_eight(struct sbc_decoder_state *state,
				struct sbc_frame *frame, int ch, int blk)
{
	int i, j, k, idx;
	int *offset = state->offset[ch];
...
__STATIC_INLINE void _sbc_analyze_four(const int16_t *in, int32_t *out)
{
	FIXED_A t1[4];
	FIXED_T t2[4];
	int i = 0, hop = 0;
...
__STATIC_INLINE void _sbc_analyze_eight(const int16_t *in, int32_t *out)
{
	FIXED_A t1[8];
	FIXED_T t2[8];
	int i, hop;
...
static int sbc_pack_frame(uint8_t *data, struct sbc_frame *frame, size_t len)
{
	...
	for (ch = 0; ch < frame->channels; ch++) {
		for (sb = 0; sb < frame->subbands; sb++) {
			...
			for (blk = 0; blk < frame->blocks; blk++) {
				while (scalefactor[ch][sb] < (uint32_t)fabs(frame->sb_sample_f[blk][ch][sb])) {
					frame->scale_factor[ch][sb]++;
					scalefactor[ch][sb] *= 2;
				}
			}
	...
	if (frame->mode == JOINT_STEREO) {
		...
		for (sb = 0; sb < frame->subbands - 1; sb++) {
			...
			for (blk = 0; blk < frame->blocks; blk++) {
				...
				while (scalefactor_j[0] < (uint32_t)fabs(sb_sample_j[blk][0])) {
					scale_factor_j[0]++;
					scalefactor_j[0] *= 2;
				}
				while (scalefactor_j[1] < (uint32_t)fabs(sb_sample_j[blk][1])) {
					scale_factor_j[1]++;
					scalefactor_j[1] *= 2;
				}
			}
...
static void sbc_set_defaults(sbc_t *sbc, uint8_t *hdr)
{
	sbc->frequency = (hdr[0] >> 6) & 0x03;
	sbc->blocks = (hdr[0] >> 4) & 0x03;
	sbc->mode = (hdr[0] >> 2) & 0x03;
	sbc->allocation = (hdr[0] >> 1) & 0x01;
	sbc->subbands = hdr[0] & 0x01;
	sbc->bitpool = hdr[1];
	sbc->endian = SBC_LE;
}

sbc_t *sbc_bluez_open(uint8_t *hdr/*2 octets*/)
{
	sbc_t *sbc;
  
	if((sbc = chHeapAlloc(0, sizeof(sbc_t))) == 0)
    return 0;

	memset(sbc, 0, sizeof(sbc_t));

	sbc->priv = chHeapAlloc(0, sizeof(struct sbc_priv));
	if (!sbc->priv)
		return 0;

	memset(sbc->priv, 0, sizeof(struct sbc_priv));

	sbc_set_defaults(sbc, hdr);
...
{
	return sbc_bluez_decode(sbc, input, input_len, NULL, 0, NULL);
}
			...
			uint16_t output_len, uint16_t *written)
{
	...
	char *ptr;
	uint16_t i, ch, framelen, samples;
	...
	if (!priv->init) {
		sbc_init(&priv->dec_state, &priv->frame);
		priv->init = 1;
	...
	for (i = 0; i < samples; i++) {
		for (ch = 0; ch < priv->frame.channels; ch++) {
				*ptr++ = (priv->frame.pcm_sample[ch][i] & 0x00ff);
				*ptr++ = (priv->frame.pcm_sample[ch][i] & 0xff00) >> 8;
		}
	}
