...
static const hs_arhythm_t g_stAudioRhythm =
{
  2,      /* rhythm led number */
  1,      /* rgb enable */
  1,      /* led active level */
...
void _led_flashRhythm(hs_arhythmcfg_t *pstARCfg)
{
  ...
  {
    if((pstARCfg->u32Cnt % 8) == 0)
    {
      u16LedSize = (pstARCfg->u32Cnt / 8) % (1u << u8RhyPinNum);
...
{
  int i;
  const hs_arhythm_t *pstAR = &g_stAudioRhythm;
