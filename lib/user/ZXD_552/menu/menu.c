...
void _menu_modeChange(uint16_t u16Msg, void *parg)
{
  ...
  if(g_eMenuMode == USER_MODE_REC)
  {
    _menu_modeRecord(u16Msg, parg);
    return ;
  }
...
void _menu_modeRecord(uint16_t u16Msg, void *parg)
{
  ...
  if(hs_recorder_isWorking())
  {
    hs_cfg_systemReq(HS_CFG_EVENT_RECORDER_SW);
  ...
  else
  {
    if(hs_fatfs_isMount(FATFS_MEMDEV_SD) || hs_fatfs_isMount(FATFS_MEMDEV_UDISK))
    {
      ...
      hs_led_disp(LED_DISP_REC);
      g_eMenuMode = USER_MODE_REC;
      hs_cfg_systemReq(HS_CFG_EVENT_RECORDER_SW);
    }
  }
}
...
void hs_menu_init(hs_usermode_t eDefaultMode)
{
  ...
  hs_cfg_sysListenMsg(USER_MSG_MODE_RECORD,  _menu_modeRecord);

  msleep(9000);
  hs_cfg_sysListenMsg(HS_CFG_EVENT_MEMDEV_IN,  _menu_memIn);  
  hs_cfg_sysListenMsg(HS_CFG_EVENT_MEMDEV_OUT, _menu_memOut); 

  hs_cfg_sysListenMsg(HS_CFG_EVENT_JACK_PLUGIN,  _menu_auxIn);  
  hs_cfg_sysListenMsg(HS_CFG_EVENT_JACK_PLUGOUT, _menu_auxOut); 
}
