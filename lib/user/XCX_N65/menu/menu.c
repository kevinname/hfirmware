...
static uint8_t g_u8UsbIsInsert;
...
void _menu_modeStart(hs_usermode_t eMode)
{
  switch(eMode)
  {
     ...
     */
    case USER_MODE_USB:
      if(g_u8UsbIsInsert == 1)
      {
        hs_cfg_sysCancelListenMsg(HS_CFG_EVENT_MEMDEV_IN,  _menu_memIn);  
        hs_cfg_sysCancelListenMsg(HS_CFG_EVENT_MEMDEV_OUT, _menu_memOut); 
...
void _menu_usbDeviceIn(uint16_t u16Msg, void *parg)
{
  (void)u16Msg;
  (void)parg;
  ...
  g_eMenuMode = USER_MODE_USB;
  _menu_modeStart(g_eMenuMode);
}

void _menu_usbDeviceOut(uint16_t u16Msg, void *parg)
{
  (void)u16Msg;
  (void)parg;

  g_u8UsbIsInsert = 0;
  if(g_eMenuMode == USER_MODE_USB)
  {
    hs_cfg_playerReq(HS_CFG_EVENT_PLAYER_DESTROY);
...
void hs_menu_init(hs_usermode_t eDefaultMode)
{
  ...
  hs_cfg_sysListenMsg(HS_CFG_EVENT_USBDEVICE_PLUGIN,   _menu_usbDeviceIn);  
  hs_cfg_sysListenMsg(HS_CFG_EVENT_USBDEVICE_PLUGOUT,  _menu_usbDeviceOut);  
}
