...
static const hs_codec_boardinfo_t g_stCodecInfo = 
{
  ...
  -13,                   /* hfp max gain of playing */
  15,                    /* adc default gain, just for hfp */
...
static const hs_padinfo_t g_stPadDefault[] = 
{
  ...
  {HS_PIN_ADC_KEY,    PAD_FUNC_GPIO,      CFG_PAD_DIR_INPUT,    CFG_PAD_PULL_DOWN,    3},
};
...
int hs_boardUsbDetect(void)
{
  ...
  for(i=0; i<u32PinNum; i++)
  {
    if((g_stPadDefault[i].u8PadMode == PAD_FUNC_SD_USB) && (g_stPadDefault[i].u8PadPull == CFG_PAD_PULL_DOWN))
    {
      if(g_stPadDefault[i].u16PadIdx >= PB0)
      {
        pstPort = IOPORT1;
        u32Pin = g_stPadDefault[i].u16PadIdx - PB0;
      }
      else
      {
        pstPort = IOPORT0;
        u32Pin = g_stPadDefault[i].u16PadIdx;
      }
      if(palReadPad(pstPort, u32Pin))
        return 1;
    }
  }
  return 0;
